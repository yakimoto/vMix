...
def process_and_send(screenshot):
    try:
        ...
        screenshot = cv2.resize(screenshot, (screen_width, screen_height), interpolation=cv2.INTER_AREA)
...
def calculate_frame_interval():
    ...
    return None if fps_option == "native" else 1.0 / float(fps_option)

def capture_and_send(frame_interval):
...
def start_capture():
    ...
    capture_and_send(frame_interval)

def stop_capture():
    start_button.config(state=tk.NORMAL, bg='green')
    error_label.config(text="Capture stopped.")
    root.quit()
...
# Retry Button
retry_button = tk.Button(root, text="Retry", command=start_capture)
